...
| 09-13 | 21 Document-model distribution over the mesh | done | content-address the L1 definition (`L1` keeps its def; `Model::definition()`; `model_def_hash` = SHA-256 of the canonical JSON); stamp every action's `ModelRef` with that hash on build (`canonical_model_ref`); a peer that receives an action under a model it lacks requests the definition over the request-response protocol (new `ModelRequest`/`ModelDef`), verifies the reply against the pinned hash, registers it, and re-applies the held action (a pending queue in the engine). Also fixed `catch_up` reporting `open@1` for every doc's `DocState.model` (now the doc's actual model). Regression: `custom_model_is_distributed_over_the_mesh` (a `task@1` doc created on a model-holding peer reaches a built-ins-only peer, fields intact, model now in its registry). |
| 09-13 | 22 Group shared space (drive + channels) + invite grant | done | `group@1` becomes a shared space: a **channel** (`post` appends to the parallel `msg_from`/`msg_text`/`msg_ts`/`msg_channel` arrays, member-gated) and a **drive** (`add-doc`/`remove-doc`, member/manager-gated) plus a `note@1` "file" model (`title`/`body`, `init`/`edit`). **Invite grant:** the daemon records `pending_invites[nonce]=groups` on `Invite` and applies `add-member` for the joiner on `InviteAccepted` (the engine emits it after verifying the join-proof), so "share a link to join a group" works end-to-end. New API: `GET /api/groups/:name`, `POST/GET /api/groups/:name/drive`, `DELETE /api/groups/:name/drive/:doc`. The console Groups tab leads with the **invite link** (copy + one-line "how to share"), then join / channel / drive / members. The live E2E caught a bug: `POST /api/groups` seeded the creator as a manager only, so the creator could not post/add-doc (member-gated) — now seeded as member+manager. 11 model unit tests (permission matrix + write contracts) + a two-daemon live E2E (create → invite → join → membership on both sides → bidirectional channel → drive file). Spec: `../../specs/2026-09-13-group-space-design.md`; evidence: `../../evidence/group-space/e2e.md`. |
...
- `cargo test -p ph-reactor -p ph-reactor-views`: all green (the lib unit
  ...
  refusal, auto-ban-on-auth-failures, model distribution, and the
  ten-client E2E).
