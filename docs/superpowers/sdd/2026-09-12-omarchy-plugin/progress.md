...
| 2026-09-12 | 7 Validation, README, preview, CI | done | README written (install, usage, IPC contract, security, dev loop); `tests/run` green (manifest validation + 15 node tests + fixture JSON + **demo preflight**: fixture CLI executed, single-line `StatusSnapshot` contract asserted — portable stand-in for the unavailable toolkit `demo_preflight.py`, negative paths verified + `omarchy plugin validate` via stub); CI runs `tests/run` on push/PR; preview image is the toolkit placeholder (final artwork deferred) |
